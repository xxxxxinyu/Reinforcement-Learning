...
class Agent():
    ...
    def learn(self):
        ...
        state = torch.as_tensor(state, dtype=None, device=None).float()
        action = np.array(action)
        action = torch.as_tensor(action, dtype=None, device=None).long()
        action = torch.reshape(action, (32, 1))
        
        next_state = np.array(next_state)
        next_state = torch.as_tensor(next_state, dtype=None, device=None).float()
                
        Q_eval = self.evaluate_net.forward(state).gather(1, action)
        Q_next = self.target_net.forward(next_state)
                
        reward = np.array(reward)
        reward = torch.as_tensor(reward, dtype=None, device=None).float()
        reward = torch.reshape(reward, (32, 1))
        done = np.array(done)
        done = torch.as_tensor(done, dtype=None, device=None).long()
        done = torch.reshape(done, (32, 1))
        one = torch.ones([32, 1], dtype=torch.long)
        Q_target = reward + self.gamma * Q_next.max(1)[0].view(self.batch_size, 1) * (one - done)
