...
class Agent():
    ...
    def discretize_observation(self, observation):
        ...
        for i, feature in enumerate(observation):
            df = self.discretize_value(feature, self.bins[i]) 
            state.append(df)
    ...
    def choose_action(self, state):
        ...
        """
        # Begin your code
        # s = self.discretize_observation(state)
    ...
    def learn(self, state, action, reward, next_state, done, ep, sum):
        """
        ...
        if ep == 2999 and sum/(ep+1) > max_reward:
    ...
    def check_max_Q(self):
        ...
        qs = self.qtable[tuple(state)]
        tmp = np.unravel_index(np.argmax(qs, axis = None), qs.shape)
        max_q = qs[tmp]
